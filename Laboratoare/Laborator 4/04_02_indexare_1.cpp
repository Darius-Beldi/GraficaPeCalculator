...
void RenderFunction(void)
{
	...
	glDrawElements(GL_LINE_LOOP, 6, GL_UNSIGNED_INT, (void*)(0));

	//patratele din stanga
	for (int i = 1; i <=2; i++) {
		myMatrix = resizeMatrix * glm::translate(glm::mat4(1.0f), glm::vec3(-30.0f * i, 0.0f, 0.0f));
		glUniformMatrix4fv(myMatrixLocation, 1, GL_FALSE, &myMatrix[0][0]);
		glDrawElements(GL_LINE_LOOP, 6, GL_UNSIGNED_INT, (void*)(0));
	}
	//patratele din dreapta
	for (int i = 1; i <= 2; i++) {
		myMatrix = resizeMatrix * glm::translate(glm::mat4(1.0f), glm::vec3(30.0f * i, 0.0f, 0.0f));
		glUniformMatrix4fv(myMatrixLocation, 1, GL_FALSE, &myMatrix[0][0]);
		glDrawElements(GL_LINE_LOOP, 6, GL_UNSIGNED_INT, (void*)(0));
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Indexarea varfurilor");		//	Creeaza fereastra de vizualizare, indicand numele acesteia;
