...
int codCol;		
//Variabila care determina clickul 
int click;
...
void Initialize(void)
{
	...
	clickLocation = glGetUniformLocation(ProgramId, "clickShader");
	randomRLocation = glGetUniformLocation(ProgramId, "randomRShader");
	randomGLocation = glGetUniformLocation(ProgramId, "randomGShader");
	randomBLocation = glGetUniformLocation(ProgramId, "randomBShader");
...
void UseMouse(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:	 
		...
		{
			click = 1;
			glUniform1i(clickLocation, click);

			randomR = (float)(rand() % 100 ) / 100;
			randomG = (float)(rand() % 100) / 100;
			randomB = (float)(rand() % 100) / 100;

			glUniform1f(randomRLocation, randomR);
			glUniform1f(randomGLocation, randomG);
			glUniform1f(randomBLocation, randomB);
